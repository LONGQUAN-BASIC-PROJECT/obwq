(function(e, i) {
	...
	function C(b, k, a) {
		...
		if (d.isFunction(k)) return d.grep(b, function(b, d) {
			...
			if (pb.test(k)) return d.filter(k, m, !a);
			else k = d.filter(k, m)
		}
	...
	function K(b, k) {
		...
		if (k.nodeType === 1) {
			...
			} else if (a === "option") k.selected = b.defaultSelected;
			else if (a === "input" || a === "textarea") k.defaultValue = b.defaultValue;
			else if (a === "script" && k.text !== b.text) k.text = b.text;
	...
	function J(b, k) {
		for (var a, m, F = [], c = 0, e = b.length; c < e; c++) if (a = b[c], a.style) if (F[c] = d._data(a, "olddisplay"), k) {
			if (!F[c] && a.style.display === "none") a.style.display = "";
	...
	function R(b, k, a, m) {
		for (var k = a === (m ? "border" : "content") ? 4 : k === "width" ? 1 : 0, F = 0; k < 4; k += 2) a === "margin" && (F += d.css(b, a + ia[k], !0)), m ? (a === "content" && (F -= parseFloat(O(b, "padding" + ia[k])) || 0), a !== "margin" && (F -= parseFloat(O(b, "border" + ia[k] + "Width")) || 0)) : (F += parseFloat(O(b, "padding" + ia[k])) || 0, a !== "padding" && (F += parseFloat(O(b, "border" + ia[k] + "Width")) || 0));
		return F
	}
	function ja(b, k, a) {
		var m = k === "width" ? b.offsetWidth : b.offsetHeight,
			F = !0,
	...
	function ca(b) {
		if (Ca[b]) return Ca[b];
		var k = d("<" + b + ">").appendTo(a.body),
			j = k.css("display");
		k.remove();
		if (j === "none" || j === "") {
			na = a.body.appendChild(na || d.extend(a.createElement("iframe"), {
				frameBorder: 0,
				width: 0,
				height: 0
			}));
	...
	function P(b, k, a, m) {
		...
		else if (!a && d.type(k) === "object") for (c in k) P(b + "[" + c + "]", k[c], a, m);
		else m(b, k)
	}
	function aa(b) {
		return function(k, a) {
			...
			var m, c, E = k.toLowerCase().split(H),
				e = 0,
				f = E.length;
	...
	function ya(b, a) {
		var k;
		var j, m, c, e, f;
		for (j in b) if (m = d.camelCase(j), c = a[m], e = b[j], d.isArray(e) && (c = e[1], k = b[j] = e[0], e = k), j !== m && (b[m] = e, delete b[j]), (f = d.cssHooks[m]) && "expand" in f) for (j in e = f.expand(e), delete b[m], e) j in b || (b[j] = e[j], a[j] = c);
		else a[m] = c
	}
	...
	function fa(b, a) {
		for (var d, m = {
			height: b
		}, c = 0, a = a ? 1 : 0; c < 4; c += 2 - a) d = ia[c], m["margin" + d] = m["padding" + d] = b;
	...
	d.fn = d.prototype = {
		...
		init: function(b, k, j) {
			...
			if (typeof b === "string") if ((m = b.charAt(0) === "<" && b.charAt(b.length - 1) === ">" && b.length >= 3 ? [null, b, null] : ua.exec(b)) && (m[1] || !k)) if (m[1]) return b = (k = k instanceof d ? k[0] : k) && k.nodeType ? k.ownerDocument || k : a, b = d.parseHTML(m[1], b, !0), ea.test(m[1]) && d.isPlainObject(k) && this.attr.call(b, k, !0), d.merge(this, b);
			else {
				...
				this.selector = b;
				return this
			} else return !k || k.jquery ? (k || j).find(b) : this.constructor(k).find(b);
			else if (d.isFunction(b)) return j.ready(b);
		...
		pushStack: function(b, a, j) {
			...
			if (a === "find") b.selector = this.selector + (this.selector ? " " : "") + j;
			else if (a) b.selector = this.selector + "." + a + "(" + j + ")";
			return b
		},
		each: function(b, a) {
	...
	d.extend = d.fn.extend = function() {
		...
		g === f && (e = this, --f);
		for (; f < g; f++) if ((b = arguments[f]) != null) for (a in b) j = e[a], m = b[a], e !== m && (h && m && (d.isPlainObject(m) || (c = d.isArray(m))) ? (c ? (c = !1, j = j && d.isArray(j) ? j : []) : j = j && d.isPlainObject(j) ? j : {}, e[a] = d.extend(h, j, m)) : m !== i && (e[a] = m));
		return e
	};
	d.extend({
		...
		merge: function(b, a) {
			var d = a.length,
				m = b.length,
				c = 0;
			if (typeof d === "number") for (; c < d; c++) b[m++] = a[c];
			else for (; a[c] !== i;) b[m++] = a[c++];
			b.length = m;
			return b
		},
		grep: function(b, a, d) {
		...
		map: function(b, a, j) {
			var m, c, e = [],
				f = 0,
				g = b.length;
			if (b instanceof d || g !== i && typeof g === "number" && (g > 0 && b[0] && b[g - 1] || g === 0 || d.isArray(b))) for (; f < g; f++) m = a(b[f], f, j), m != null && (e[e.length] = m);
			else for (c in b) m = a(b[c], c, j), m != null && (e[e.length] = m);
			return e.concat.apply([], e)
		},
		guid: 1,
		proxy: function(b, a) {
			var j, m;
			typeof a === "string" && (j = b[a], a = b, b = j);
		...
		access: function(b, a, j, m, c, e, f) {
			...
			return c ? b : h ? a.call(b) : w ? a(b[0], j) : e
		},
		now: function() {
	...
	d.ready.promise = function(b) {
		if (!T) if (T = d.Deferred(), a.readyState === "complete") setTimeout(d.ready, 1);
		else if (a.addEventListener) a.addEventListener("DOMContentLoaded", ra, !1), e.addEventListener("load", d.ready, !1);
		else {
	...
	d.Callbacks = function(b) {
		var b = typeof b === "string" ? Ma[b] || h(b) : d.extend({}, b),
			a, j, m, c, e, f, g = [],
			l = !b.once && [],
			w = function(d) {
				a = b.memory && d;
			...
			x = {
				...
				remove: function() {
					g && d.each(arguments, function(b, a) {
						for (var k;
						(k = d.inArray(a, g, k)) > -1;) g.splice(k, 1), m && (k <= e && e--, k <= f && f--)
					});
					return this
				},
				has: function(b) {
	...
	d.extend({
					...
					then: function() {
						...
						return d.Deferred(function(j) {
							d.each(a, function(a, k) {
								var m = k[0],
									e = b[a];
								c[k[1]](d.isFunction(e) ?
								function() {
									var b = e.apply(this, arguments);
									if (b && d.isFunction(b.promise)) b.promise().done(j.resolve).fail(j.reject).progress(j.notify);
									else j[m + "With"](this === c ? j : this, [b])
								} : j[m])
							});
							b = null
						}).promise()
		...
		when: function(b) {
			var a = 0,
				j = t.call(arguments),
				m = j.length,
				c = m !== 1 || b && d.isFunction(b.promise) ? m : 0,
				e = c === 1 ? b : d.Deferred(),
				f = function(b, a, d) {
					return function(k) {
						a[b] = this;
			...
			if (m > 1) {
				g = Array(m);
				i = Array(m);
				for (h = Array(m); a < m; a++) j[a] && d.isFunction(j[a].promise) ? j[a].promise().done(f(a, h, j)).fail(e.reject).progress(f(a, i, g)) : --c
			}
			c || e.resolveWith(h, j);
			return e.promise()
		}
	...
	d.support = function() {
		...
		b = {
			...
			html5Clone: a.createElement("nav").cloneNode(!0).outerHTML !== "<:nav></:nav>",
		...
		j.removeChild(k);
		j.appendChild(i);
		if (i.attachEvent) for (f in {
			submit: !0,
			change: !0,
			focusin: !0
		}) k = "on" + f, g = k in i, g || (i.setAttribute(k, "return;"), g = typeof i[k] === "function"), b[f + "Bubbles"] = g;
		d(function() {
			var d, k, j, m = a.getElementsByTagName("body")[0];
			if (m) {
				...
				b.reliableHiddenOffsets = g && j[0].offsetHeight === 0;
				k.innerHTML = "";
				...
				if (e.getComputedStyle) b.pixelPosition = (e.getComputedStyle(k, null) || {}).top !== "1%", b.boxSizingReliable = (e.getComputedStyle(k, null) || {
					width: "4px"
				}).width === "4px", j = a.createElement("div"), j.style.cssText = k.style.cssText = "padding:0;margin:0;border:0;display:block;overflow:hidden;", j.style.marginRight = j.style.width = "0", k.style.width = "1px", k.appendChild(j), b.reliableMarginRight = !parseFloat((e.getComputedStyle(j, null) || {}).marginRight);
				if (typeof k.style.zoom !== "undefined") k.innerHTML = "", k.style.cssText = "padding:0;margin:0;border:0;display:block;overflow:hidden;width:1px;padding:1px;display:inline;zoom:1", b.inlineBlockNeedsLayout = k.offsetWidth === 3, k.style.display = "block", k.style.overflow = "visible", k.innerHTML = "<div></div>", k.firstChild.style.width = "5px", b.shrinkWrapBlocks = k.offsetWidth !== 3, d.style.zoom = 1;
				m.removeChild(d)
			}
		});
		j.removeChild(i);
	...
	d.extend({
		...
		removeData: function(b, a, j) {
			if (d.acceptData(b)) {
				var m, c, e, f = b.nodeType,
					g = f ? d.cache : b,
					i = f ? b[d.expando] : d.expando;
				if (g[i]) {
	...
	d.fn.extend({
		data: function(b, a) {
			...
			return d.access(this, function(a) {
				if (a === i) return l = this.triggerHandler("getData" + m, [j[0]]), l === i && g && (l = d.data(g, b), l = u(g, b, l)), l === i && j[1] ? this.data(j[0]) : l;
				j[1] = a;
				this.each(function() {
					var k = d(this);
	...
	d.extend({
		...
		_queueHooks: function(b, a) {
			var j = a + "queueHooks";
			return d._data(b, j) || d._data(b, j, {
	...
	d.fn.extend({
		...
		addClass: function(b) {
			...
			if (b && typeof b === "string") {
				a = b.split(H);
				j = 0;
				for (m = this.length; j < m; j++) if (c = this[j], c.nodeType === 1) if (!c.className && a.length === 1) c.className = b;
				else {
					e = " " + c.className + " ";
					f = 0;
		...
		removeClass: function(b) {
			...
			if (b && typeof b === "string" || b === i) {
				...
				for (g = this.length; f < g; f++) if (m = this[f], m.nodeType === 1 && m.className) {
					...
					c = 0;
					for (e = a.length; c < e; c++) for (; j.indexOf(" " + a[c] + " ") >= 0;) j = j.replace(" " + a[c] + " ", " ");
		...
		toggleClass: function(b, a) {
			...
			return this.each(function() {
				if (j === "string") for (var e, f = 0, g = d(this), i = a, h = b.split(H); e = h[f++];) i = c ? i : !g.hasClass(e), g[i ? "addClass" : "removeClass"](e);
				else if (j === "undefined" || j === "boolean") this.className && d._data(this, "__className__", this.className), this.className = this.className || b === !1 ? "" : d._data(this, "__className__") || ""
			})
		},
		hasClass: function(b) {
	...
	d.extend({
		valHooks: {
			...
			select: {
				get: function(b) {
					for (var a, j = b.options, c = b.selectedIndex, e = (b = b.type === "select-one" || c < 0) ? null : [], f = b ? c + 1 : j.length, g = c < 0 ? f : b ? c : 0; g < f; g++) if (a = j[g], (a.selected || g === c) && (d.support.optDisabled ? !a.disabled : a.getAttribute("disabled") === null) && (!a.parentNode.disabled || !d.nodeName(a.parentNode, "optgroup"))) {
						a = d(a).val();
						if (b) return a;
						e.push(a)
					}
		...
		attr: function(b, a, j, c) {
			...
			if (b && !(g === 3 || g === 8 || g === 2)) {
				...
				if (j !== i) if (j === null) d.removeAttr(b, a);
				else return f && "set" in f && c && (e = f.set(b, j, a)) !== i ? e : (b.setAttribute(a, j + ""), j);
				else return f && "get" in f && c && (e = f.get(b, a)) !== null ? e : (e = b.getAttribute(a), e === null ? i : e)
			}
		},
		removeAttr: function(b, a) {
			var j, c, e, f, g = 0;
			if (a && b.nodeType === 1) for (c = a.split(H); g < c.length; g++) if (e = c[g]) j = d.propFix[e] || e, (f = Ua.test(e)) || d.attr(b, e, ""), b.removeAttribute(Va ? e : j), f && j in b && (b[j] = !1)
		},
		attrHooks: {
			type: {
				set: function(b, a) {
					if (wb.test(b.nodeName) && b.parentNode) d.error("type property can't be changed");
			...
			value: {
				...
				set: function(b, a, j) {
					if (ha && d.nodeName(b, "button")) return ha.set(b, a, j);
					b.value = a
				}
			}
		},
		...
		prop: function(b, a, j) {
			...
			if (b && !(f === 3 || f === 8 || f === 2)) {
				...
				return j !== i ? e && "set" in e && (c = e.set(b, j, a)) !== i ? c : b[a] = j : e && "get" in e && (c = e.get(b, a)) !== null ? c : b[a]
			}
		},
		propHooks: {
			tabIndex: {
	...
	var Ga = /^(?:textarea|input|select)$/i,
		...
		Ya = function(b) {
			return d.event.special.hover ? b : b.replace(zb, "mouseenter$1 mouseleave$1")
		};
	d.event = {
		add: function(b, a, j, c, e) {
			...
			if (!(b.nodeType === 3 || b.nodeType === 8 || !a || !j || !(f = d._data(b)))) {
				...
				for (f = 0; f < a.length; f++) {
					...
					e ? p.splice(p.delegateCount++, 0, l) : p.push(l);
					d.event.global[w] = !0
				}
				b = null
			}
		...
		remove: function(b, a, j, c, e) {
			...
			if (q && (x = q.events)) {
				...
				for (f = 0; f < a.length; f++) if (g = Wa.exec(a[f]) || [], i = h = g[1], g = g[2], i) {
					...
					for (w = 0; w < p.length; w++) if (t = p[w], (e || h === t.origType) && (!j || j.guid === t.guid) && (!g || g.test(t.namespace)) && (!c || c === t.selector || c === "**" && t.selector)) p.splice(w--, 1), t.selector && p.delegateCount--, n.remove && n.remove.call(b, t);
		...
		dispatch: function(b) {
			var b = d.event.fix(b || e.event),
				a, j, c, f, g, h, l = (d._data(this, "events") || {})[b.type] || [],
				w = l.delegateCount,
				n = t.call(arguments),
				x = !b.exclusive && !b.namespace,
				p = d.event.special[b.type] || {},
				q = [];
			n[0] = b;
	...
	d.removeEvent = a.removeEventListener ?
	function(b, a, d) {
	...
	d.Event = function(b, a) {
		...
		b && b.type ? (this.originalEvent = b, this.type = b.type, this.isDefaultPrevented = b.defaultPrevented || b.returnValue === !1 || b.getPreventDefault && b.getPreventDefault() ? z : B) : this.type = b;
		a && d.extend(this, a);
	...
	d.fn.extend({
		on: function(b, a, j, c, e) {
			...
			j == null && c == null ? (c = a, j = a = i) : c == null && (typeof a === "string" ? (c = j, j = i) : (c = j, j = a, a = i));
			if (c === !1) c = B;
		...
		undelegate: function(b, a, d) {
			return arguments.length === 1 ? this.off(b, "**") : this.off(a, b || "**", d)
		},
	...
	(function(b, a) {
		function j(b, a, d, j) {
			...
			} else if (c[2]) return ea.apply(d, K.call(a.getElementsByTagName(b), 0)), d;
			else if ((f = c[3]) && T && a.getElementsByClassName) return ea.apply(d, K.call(a.getElementsByClassName(f), 0)), d;
		...
		function h(b, a, d) {
			...
			function(a, d, e) {
				...
				} else for (var f, m = ga + " " + e + " ", g = m + $; a = a[c];) if (j || a.nodeType === 1) if ((f = a[v]) === g) return a.sizset;
				else if (typeof f === "string" && f.indexOf(m) === 0) {
		...
		function w(b, a, d, c, j) {
			for (var e, k = [], f = 0, m = b.length, g = a != null; f < m; f++) if (e = b[f]) if (!d || d(e, c, j)) k.push(e), g && a.push(f);
			return k
		}
			...
			return Z(function(k, f, m, g) {
				...
				if (k) {
					if (e || b) {
						if (e) {
							i = [];
							for (m = E.length; m--;) if (h = E[m]) i.push(F[m] = h);
							e(null, E = [], i, g)
						}
		...
		function x(b) {
			var a, d, c, j = b.length,
				e = o.relative[b[0].type];
			d = e || o.relative[" "];
			...
			}]; k < j; k++) if (d = o.relative[b[k].type]) g = [h(l(g), d)];
			else {
				d = o.filter[b[k].type].apply(null, b[k].matches);
				if (d[v]) {
			...
			var d = a.length > 0,
				...
				e = function(k, f, m, g, i) {
					var h, l, n = [],
						...
						p = i != null,
						t = A,
						ma = k || c && o.find.TAG("*", i && f.parentNode || f),
						q = ga += t == null ? 1 : Math.E;
					if (p) A = f !== W && f, $ = e.el;
					for (;
					(i = ma[x]) != null; x++) {
						if (c && i) {
		...
		function t(b, a, d, c, j) {
			...
			if (!c && g.length === 1) {
				...
				for (e = aa.POS.test(b) ? -1 : k.length - 1; e >= 0; e--) {
					f = k[e];
		...
		function q() {}
		var $, z, o, r, H, U, D, s, X, A, u = !0,
			v = ("sizcache" + Math.random()).replace(".", ""),
			C = String,
			W = b.document,
			y = W.documentElement,
			ga = 0,
		...
		}, Q = I(), ca = I(), za = I(), I = "\\[[\\x20\\t\\r\\n\\f]*((?:\\\\.|[-\\w]|[^\\x00-\\xa0])+)[\\x20\\t\\r\\n\\f]*(?:([*^$|!~]?=)[\\x20\\t\\r\\n\\f]*(?:(['\"])((?:\\\\.|[^\\\\])*?)\\3|(" + "(?:\\\\.|[-\\w]|[^\\x00-\\xa0])+".replace("w", "w#") + ")|)|)[\\x20\\t\\r\\n\\f]*\\]", P = ":((?:\\\\.|[-\\w]|[^\\x00-\\xa0])+)(?:\\((?:(['\"])((?:\\\\.|[^\\\\])*?)\\2|([^()[\\]]*|(?:(?:" + I + ")|[^:]|\\\\.)*|.*))\\)|)", J = RegExp("^[\\x20\\t\\r\\n\\f]+|((?:^|[^\\\\])(?:\\\\.)*)[\\x20\\t\\r\\n\\f]+$", "g"), L = /^[\x20\t\r\n\f]*,[\x20\t\r\n\f]*/, S = /^[\x20\t\r\n\f]*([\x20\t\r\n\f>+~])[\x20\t\r\n\f]*/, M = RegExp(P), Ea = /^(?:#([\w\-]+)|(\w+)|\.([\w\-]+))$/, ba = /[\x20\t\r\n\f]*[+~]/, da = /h\d/i, O = /input|select|textarea|button/i, R = /\\(?!\\)/g, aa = {
			ID: /^#((?:\\.|[-\w]|[^\x00-\xa0])+)/,
			CLASS: /^\.((?:\\.|[-\w]|[^\x00-\xa0])+)/,
			NAME: /^\[name=['"]?((?:\\.|[-\w]|[^\x00-\xa0])+)['"]?\]/,
		...
		r = j.getText = function(b) {
			var a, d = "",
				c = 0;
			if (a = b.nodeType) if (a === 1 || a === 9 || a === 11) if (typeof b.textContent === "string") return b.textContent;
			else for (b = b.firstChild; b; b = b.nextSibling) d += r(b);
			else {
		...
		H = j.isXML = function(b) {
			return (b = b && (b.ownerDocument || b).documentElement) ? b.nodeName !== "HTML" : !1
		};
		U = j.contains = y.contains ?
		function(b, a) {
		...
		j.attr = function(b, a) {
			...
			return (d = b.getAttributeNode(a)) ? typeof b[a] === "boolean" ? b[a] ? a : null : d.specified ? d.value : null : null
		};
		o = j.selectors = {
			cacheLength: 50,
			createPseudo: Z,
			...
			find: {
				...
				function(b, a, d) {
					...
					if (typeof d.getElementById !== "undefined" && !c) return (d = d.getElementById(b)) ? d.id === b || typeof d.getAttributeNode !== "undefined" && d.getAttributeNode("id").value === b ? [d] : a : []
				},
				TAG: I ?
				function(b, a) {
			...
			filter: {
				...
				ATTR: function(b, a, d) {
					return function(c) {
						...
						if (!a) return !0;
						c += "";
						return a === "=" ? c === d : a === "!=" ? c !== d : a === "^=" ? d && c.indexOf(d) === 0 : a === "*=" ? d && c.indexOf(d) > -1 : a === "$=" ? d && c.substr(c.length - d.length) === d : a === "~=" ? (" " + c + " ").indexOf(d) > -1 : a === "|=" ? c === d || c.substr(0, d.length + 1) === d + "-" : !1
					}
				},
			...
			pseudos: {
				not: Z(function(b) {
					...
					return c[v] ? Z(function(b, a, d, j) {
						for (var j = c(b, null, j, []), k = b.length; k--;) if (d = j[k]) b[k] = !(a[k] = d)
					}) : function(b, j, k) {
						a[0] = b;
						c(a, null, k, d);
						return !d.pop()
		...
		function() {
			var b, a = t,
				...
				e = [":focus"],
				k = [":active"],
				f = y.matchesSelector || y.mozMatchesSelector || y.webkitMatchesSelector || y.oMatchesSelector || y.msMatchesSelector;
			N(function(b) {
			...
			t = function(b, c, j, k, f) {
				if (!k && !f && !e.test(b)) {
					...
					if (c.nodeType === 1 && c.nodeName.toLowerCase() !== "object") {
						m = i(b);
						(h = c.getAttribute("id")) ? l = h.replace(d, "\\$&") : c.setAttribute("id", l);
						l = "[id='" + l + "'] ";
			...
			if (f) N(function(a) {
				...
				if (!H(a) && !k.test(d) && !e.test(d)) try {
					var m = f.call(a, d);
					if (m || b || a.document && a.document.nodeType !== 11) return m
				} catch (g) {}
	...
	d.fn.extend({
		...
		add: function(b, a) {
			var c = typeof b === "string" ? d(b, a) : d.makeArray(b && b.nodeType ? [b] : b),
	...
	var Na = "abbr|article|aside|audio|bdi|canvas|data|datalist|details|figcaption|figure|footer|header|hgroup|mark|meter|nav|output|progress|section|summary|time|video",
		...
		cb = v(a),
		Ja = cb.appendChild(a.createElement("div"));
	Y.optgroup = Y.option;
	...
	d.fn.extend({
		text: function(b) {
			return d.access(this, function(b) {
				return b === i ? d.text(this) : this.empty().append((this[0] && this[0].ownerDocument || a).createTextNode(b))
		...
		remove: function(b, a) {
			for (var c, e = 0;
			(c = this[e]) != null; e++) if (!b || d.filter(b, [c]).length)!a && c.nodeType === 1 && (d.cleanData(c.getElementsByTagName("*")), d.cleanData([c])), c.parentNode && c.parentNode.removeChild(c);
			return this
		},
		empty: function() {
			for (var b, a = 0;
		...
		domManip: function(b, a, c) {
			...
			if (this[0]) {
				...
				if (f) {
					a = a && d.nodeName(f, "tr");
					for (e = e.cacheable || n - 1; h < n; h++) c.call(a && d.nodeName(this[h], "table") ? this[h].getElementsByTagName("tbody")[0] || this[h].appendChild(this[h].ownerDocument.createElement("tbody")) : this[h], h === e ? g : d.clone(g, !0, !0))
				}
				g = f = null;
				w.length && d.each(w, function(b, a) {
					a.src ? d.ajax ? d.ajax({
						...
						global: !1,
						"throws": !0
					}) : d.error("no ajax") : d.globalEval((a.text || a.textContent || a.innerHTML || "").replace(Lb, ""));
	...
	d.buildFragment = function(b, c, e) {
		var f, g, h, l = b[0],
			...
			c = !c.nodeType && c[0] || c,
			c = c.ownerDocument || c;
		if (b.length === 1 && typeof l === "string" && l.length < 512 && c === a && l.charAt(0) === "<" && !Jb.test(l) && (d.support.checkClone || !bb.test(l)) && (d.support.html5Clone || !Ia.test(l))) g = !0, f = d.fragments[l], h = f !== i;
	...
	d.each({
		...
		d.fn[b] = function(c) {
			...
			if ((e == null || e && e.nodeType === 11 && e.childNodes.length === 1) && i === 1) return c[a](this[0]), this;
			else {
	...
	d.extend({
		...
		clean: function(b, c, e, f) {
			var g, i, h, l, w, n, x = c === a && cb,
				p = [];
			if (!c || typeof c.createDocumentFragment === "undefined") c = a;
			for (g = 0;
			(h = b[g]) != null; g++) if (typeof h === "number" && (h += ""), h) {
			...
			n && (h = n = x = null);
			if (!d.support.appendChecked) for (g = 0;
			(h = p[g]) != null; g++) d.nodeName(h, "input") ? I(h) : typeof h.getElementsByTagName !== "undefined" && d.grep(h.getElementsByTagName("input"), I);
			if (e) {
				b = function(b) {
					if (!b.type || Kb.test(b.type)) return f ? f.push(b.parentNode ? b.parentNode.removeChild(b) : b) : e.appendChild(b)
				};
				for (g = 0;
				(h = p[g]) != null; g++) if (!d.nodeName(h, "script") || !b(h)) e.appendChild(h), typeof h.getElementsByTagName !== "undefined" && (h = d.grep(d.merge([], h.getElementsByTagName("script")), b), p.splice.apply(p, [g + 1, 0].concat(h)), g += h.length)
			}
			return p
		},
		cleanData: function(b, a) {
			for (var c, e, f, g, h = 0, i = d.expando, l = d.cache, n = d.support.deleteExpando, w = d.event.special;
			(f = b[h]) != null; h++) if (a || d.acceptData(f)) if (c = (e = f[i]) && l[e]) {
	...
	(function() {
		...
		if (c.chrome) c.webkit = !0;
		else if (c.webkit) c.safari = !0;
		d.browser = c;
		d.sub = function() {
			function b(a, d) {
			...
			b.fn.init = function(a, e) {
				...
				return d.fn.init.call(this, a, e, c)
			};
	...
	d.extend({
		...
		style: function(b, a, c, e) {
			if (b && !(b.nodeType === 3 || b.nodeType === 8 || !b.style)) {
				var f, g, h, l = d.camelCase(a),
					n = b.style,
					a = d.cssProps[l] || (d.cssProps[l] = M(n, l));
				h = d.cssHooks[a] || d.cssHooks[l];
				if (c !== i) {
					g = typeof c;
					if (g === "string" && (f = Pb.exec(c))) c = (f[1] + 1) * f[2] + parseFloat(d.css(b, a)), g = "number";
					if (!(c == null || g === "number" && isNaN(c))) if (g === "number" && !d.cssNumber[l] && (c += "px"), !h || !("set" in h) || (c = h.set(b, c, e)) !== i) try {
						n[a] = c
					} catch (w) {}
				} else {
		...
		swap: function(b, a, d) {
			...
			d = d.call(b);
			for (c in a) b.style[c] = e[c];
			return d
		}
	});
	e.getComputedStyle ? O = function(b, a) {
		var c, f, g, h, i = e.getComputedStyle(b, null),
			l = b.style;
		...
		var d, c, e = b.currentStyle && b.currentStyle[a],
			f = b.style;
		e == null && f && f[a] && (e = f[a]);
	...
	if (!d.support.opacity) d.cssHooks.opacity = {
		...
		set: function(b, a) {
			var c = b.style,
				e = b.currentStyle,
				f = d.isNumeric(a) ? "alpha(opacity=" + a * 100 + ")" : "",
	...
	d.param = function(b, a) {
		...
		a === i && (a = d.ajaxSettings && d.ajaxSettings.traditional);
	...
	try {
		...
		la = a.createElement("a"), la.href = "", la = la.href
	}
		...
		ajax: function(b, a) {
			function c(b, a, g, j) {
				...
				if (U !== 2) {
					...
					if (b >= 200 && b < 300 || b === 304) if (p.ifModified && ((g = s.getResponseHeader("Last-Modified")) && (d.lastModified[e] = g), (g = s.getResponseHeader("Etag")) && (d.etag[e] = g)), b === 304) r = "notmodified", k = !0;
					else {
						a: {
							n = p;
							x = E;
							...
							for (; g = r[++D];) if (g !== "*") {
								if (v !== "*" && v !== g) {
									...
									if (C !== !0) if (C && n["throws"]) x = C(x);
									else try {
										x = C(x)
									} catch (ga) {
										k = {
											state: "parsererror",
											error: C ? ga : "No conversion from " + v + " to " + g
										};
										break a
									}
								}
					...
					s.statusCode(z);
					z = i;
					w && q.trigger("ajax" + (k ? "Success" : "Error"), [s, p, k ? n : x]);
					$.fireWith(t, [s, r]);
			...
			var a = a || {},
				e, f, g, h, l, n, w, x, p = d.ajaxSetup({}, a),
				t = p.context || p,
				q = t !== p && (t.nodeType || t instanceof d) ? d(t) : d.event,
				o = d.Deferred(),
			...
			s.statusCode = function(b) {
				if (b) {
					var a;
					if (U < 2) for (a in b) z[a] = [z[a], b[a]];
					else a = b[s.status], s.always(a)
				}
				return this
			};
			p.url = ((b || p.url) + "").replace(Vb, "").replace(Yb, ka[1] + "//");
			...
			if (!p.hasContent && (p.data && (p.url += (gb.test(p.url) ? "&" : "?") + p.data, delete p.data), e = p.url, p.cache === !1)) {
				n = d.now();
			...
			s.setRequestHeader("Accept", p.dataTypes[0] && p.accepts[p.dataTypes[0]] ? p.accepts[p.dataTypes[0]] + (p.dataTypes[0] !== "*" ? ", " + kb + "; q=0.01" : "") : p.accepts["*"]);
	...
	d.ajaxPrefilter("json jsonp", function(b, a, c) {
		var f, g, h, l = b.data,
			...
			w = b.jsonp !== !1,
			p = w && Aa.test(n),
			x = w && !p && typeof l === "string" && !(b.contentType || "").indexOf("application/x-www-form-urlencoded") && Aa.test(l);
		if (b.dataTypes[0] === "jsonp" || p || x) return f = b.jsonpCallback = d.isFunction(b.jsonpCallback) ? b.jsonpCallback() : b.jsonpCallback, g = e[f], p ? b.url = n.replace(Aa, "$1" + f) : x ? b.data = l.replace(Aa, "$1" + f) : w && (b.url += (ac.test(n) ? "&" : "?") + b.jsonp + "=" + f), b.converters["script json"] = function() {
	...
	var sa, La = e.ActiveXObject ?
	function() {
		for (var b in sa) sa[b](0, 1)
	} : !1, cc = 0;
	d.ajaxSettings.xhr = e.ActiveXObject ?
	function() {
		var b;
		...
		if (!b.crossDomain || d.support.cors) {
			...
			return {
				send: function(c, f) {
					...
					try {
						for (h in c) l.setRequestHeader(h, c[h])
					} catch (n) {}
					...
					b.async ? l.readyState === 4 ? setTimeout(a, 0) : (g = ++cc, La && (sa || (sa = {}, d(e).unload(La)), sa[g] = a), l.onreadystatechange = a) : a()
				},
				abort: function() {
					a && a(0, 1)
				}
	...
	var pa, Ba, dc = /^(?:toggle|show|hide)$/,
		ec = RegExp("^(?:([-+])=|)(" + $ + ")([a-z%]*)$", "i"),
		fc = /queueHooks$/,
		xa = [function(b, a, c) {
			var e, f, g, h, i, l, n = this,
				w = b.style,
				p = {},
				x = [],
				t = b.nodeType && y(b);
			...
			for (e in a) g = a[e], dc.exec(g) && (delete a[e], f = f || g === "toggle", g !== (t ? "hide" : "show") && x.push(e));
			if (a = x.length) {
		...
		ta = {
			"*": [function(b, a) {
				var c, e, f = this.createTween(b, a),
					g = ec.exec(a),
					h = f.cur(),
					i = +h || 0,
					l = 1,
				...
				if (g) {
					...
					if (e !== "px" && i) {
						...
						do l = l || ".5", i /= l, d.style(f.elem, b, i + e);
						while (l !== (l = f.cur() / h) && l !== 1 && --n)
					}
					f.unit = e;
					f.start = i;
	...
	L.propHooks = {
		_default: {
			...
			set: function(a) {
				...
				else a.elem.style && (a.elem.style[d.cssProps[a.prop]] != null || d.cssHooks[a.prop]) ? d.style(a.elem, a.prop, a.now + a.unit) : a.elem[a.prop] = a.now
			}
		}
	};
		...
		stop: function(a, c, e) {
			...
			return this.each(function() {
				var c = !0,
					...
					h = d.timers,
					i = d._data(this);
				if (g) i[g] && i[g].stop && f(i[g]);
				else for (g in i) i[g] && i[g].stop && fc.test(g) && f(i[g]);
				for (g = h.length; g--;) if (h[g].elem === this && (a == null || h[g].queue === a)) h[g].anim.stop(e), c = !1, h.splice(g, 1);
	...
	d.speed = function(a, c, e) {
		var f = a && typeof a === "object" ? d.extend({}, a) : {
			complete: e || !e && c || d.isFunction(a) && a,
			duration: a,
			easing: e && c || c && !d.isFunction(c) && c
		};
	...
	d.fn.offset = function(a) {
		...
		if (g = (f = this[0]) && f.ownerDocument) {
			...
			return {
				top: l.top + h - g,
				left: l.left + (f.pageXOffset || c.scrollLeft) - e
			}
		}
	};
	d.offset = {
		bodyOffset: function(a) {
			var c = a.offsetTop,
				e = a.offsetLeft;
			d.support.doesNotIncludeMarginInBodyOffset && (c += parseFloat(d.css(a, "marginTop")) || 0, e += parseFloat(d.css(a, "marginLeft")) || 0);
			return {
				top: c,
				left: e
			}
		...
		setOffset: function(a, c, e) {
			...
			if (c.left != null) n.left = c.left - h.left + l;
	...
	d.fn.extend({
		...
		offsetParent: function() {
			return this.map(function() {
				for (var b = this.offsetParent || a.body; b && !mb.test(b.nodeName) && d.css(b, "position") === "static";) b = b.offsetParent;
				return b || a.body
			})
		}
	});
	...
	d.each({
		...
		d.each({
			...
			d.fn[f] = function(f, g) {
				var h = arguments.length && (e || typeof f !== "boolean"),
					l = e || (f === !0 || g === !0 ? "margin" : "border");
...
(function(e, i, h, u) {
	var o = i.document,
		...
		D = i.location.hash.replace(/#\//, ""),
		K = i.location.protocol,
		r = Math,
		I = function() {},
		M = function() {
			return !1
		...
		da = function(a) {
			var c, e;
			for (e in P) if ((c = a && P[e].reg && a.match(P[e].reg)) && c.length) return {
				id: c[2],
				provider: e
			};
			return !1
		...
		fa = function(a) {
			...
			e.each(ya, function(c, e) {
				if (e._options.theme == a.name || !e._initialized && !e._options.theme) e.theme = a, e._init.call(e)
			})
		},
		q = function() {
			return {
				...
				animate: function() {
					var a = function(a) {
							var d = "transition WebkitTransition MozTransition OTransition".split(" "),
								c;
							if (i.opera) return !1;
						...
						r.duration ? a ? (r.stop && (o.off(c), l(o)), n = !1, e.each(z, function(a, c) {
							...
							return function() {
								...
								if (h.WEBKIT && h.TOUCH && (w = {}, x = [0, 0, 0], e.each(["left", "top"], function(d, e) {
									e in c && (x[d] = q.parseValue(c[e]) - q.parseValue(a.css(e)) + "px", w[e] = c[e], delete c[e])
				...
				hide: function(a, c, g) {
					...
					c ? q.animate(a, h, {
						...
						stop: !0
					}) : y < 9 && a ? (q.removeAlpha(a), a.style.visibility = "hidden") : f.css(h)
				},
				show: function(a, c, g) {
					var g = g || I,
						f = e(a),
						a = f[0],
						h = {
							opacity: parseFloat(f.data("opacity")) || 1
						};
					if (c) {
				...
				insertStyleTag: function(a, c) {
					if (!c || !e("#" + c).length) {
						...
						if (g.styleSheet) g.styleSheet.cssText = a;
						else {
				...
				loadCSS: function(a, c, g) {
					...
					e("link[rel=stylesheet]").each(function() {
						if (RegExp(a).test(this.href)) return f = this, !1
					});
					typeof c === "function" && (g = c, c = u);
					g = g || I;
					if (f) return g.call(f, f), f;
					i = o.styleSheets.length;
					if (e("#" + c).length) e("#" + c).attr("href", a), i--;
					else if (f = e("<link>").attr({
						rel: "stylesheet",
						href: a,
						id: c
					}).get(0), c = e('link[rel="stylesheet"], style'), c.length ? c.get(0).parentNode.insertBefore(f, c[0]) : J().head.appendChild(f), y && i >= 31) {
		...
		T = function() {
			var a = function(a, g, f, h) {
					var i = this.getOptions("easing"),
						t = this.getStageWidth(),
						p = {
							left: t * (a.rewind ? -1 : 1)
						},
						o = {
			...
			return {
				...
				effects: {
					fade: function(a, g) {
						...
						a.prev && (e(a.prev).css("opacity", 1).show(), q.animate(a.prev, {
							opacity: 0
						}, {
							duration: a.speed
						}))
	...
	e.event.special["click:fast"] = {
		...
		add: function(a) {
			var c = function(a) {
					if (a.touches && a.touches.length) return a = a.touches[0], {
						x: a.pageX,
						y: a.pageY
					}
				},
			...
			e(this).data({
				clickstate: g,
				timer: 0
			}).on("touchstart.fast", function(a) {
				i.clearTimeout(e(this).data("timer"));
				e(this).data("clickstate", {
					touched: !0,
					touchdown: !0,
					coords: c(a.originalEvent),
					evObj: a
				})
			}).on("touchmove.fast", function(a) {
				var a = c(a.originalEvent),
					g = e(this).data("clickstate");
				Math.max(Math.abs(g.coords.x - a.x), Math.abs(g.coords.y - a.y)) > 6 && e(this).data("clickstate", e.extend(g, {
					touchdown: !1
				}))
	...
	h = function() {
		...
		var c = this._keyboard = {
			...
			f = this._carousel = {
				...
				set: function(a) {
					for (a = r.max(a, 0); f.hooks[a - 1] + f.width >= f.max && a >= 0;) a--;
					f.current = a;
					f.animate()
				},
				getLast: function(a) {
				...
				follow: function(a) {
					if (a === 0 || a === f.hooks.length - 2) f.set(a);
					else {
			...
			l = this._tooltip = {
				...
				init: function() {
					l.initialized = !0;
					q.insertStyleTag(".galleria-tooltip{padding:3px 8px;max-width:50%;background:#ffe;color:#000;z-index:3;position:absolute;font-size:11px;line-height:1.3;opacity:0;box-shadow:0 0 2px rgba(0,0,0,.4);-moz-box-shadow:0 0 2px rgba(0,0,0,.4);-webkit-box-shadow:0 0 2px rgba(0,0,0,.4);}", "galleria-tooltip");
					a.$("tooltip").css({
						opacity: 0.8,
						visibility: "visible",
						display: "none"
				...
				move: function(c) {
					var e = a.getMousePosition(c).x,
						...
						h = d.outerWidth(!0),
						i = g + 15,
						h = a.$("container").width() - h - 2,
						l = a.$("container").height() - g - 2;
					!isNaN(e) && !isNaN(f) && (e += 10, f -= g + 8, e = r.max(0, r.min(h, e)), f = r.max(0, r.min(l, f)), c < i && (f = i), d.css({
						left: e,
						top: f
					}))
				},
				bind: function(c, f) {
					if (!h.TOUCH) {
						...
						var d = function() {
								a.$("container").off("mousemove", l.move);
						...
						typeof f === "string" ? g(c in a._dom ? a.get(c) : c, f) : e.each(c, function(d, c) {
							g(a.get(d), c)
						})
					}
				},
				show: function(c) {
					var c = e(c in a._dom ? a.get(c) : c),
						f = c.data("tt"),
						d = function(a) {
			...
			n = this._fullscreen = {
				...
				parseCallback: function(c, f) {
					return T.active ?
					function() {
						typeof c == "function" && c.call(a);
						var d = a._controls.getActive(),
							g = a._controls.getNext();
						a._scaleImage(g);
						a._scaleImage(d);
				...
				enter: function(c) {
					n.beforeEnter(function() {
						...
						a._options.trueFullscreen && S.support ? (n.active = !0, q.forceStyles(a.get("container"), {
							width: "100%",
							height: "100%"
						}), a.rescale(), h.MAC ? !h.SAFARI || !/version\/[1-5]/.test(v) ? (a.$("container").css("opacity", 0).addClass("fullscreen"), i.setTimeout(function() {
							n.scale();
				...
				_enter: function(c) {
					...
					var f = a.getData(),
						d = a._options,
						g = !a._options.trueFullscreen || !S.support,
						l = {
							height: "100%",
							overflow: "hidden",
							margin: 0,
					...
					if (g) {
						...
						q.forceStyles(a.get("container"), {
							position: h.TOUCH ? "absolute" : "fixed",
							top: 0,
							left: 0,
							width: "100%",
							height: "100%",
					...
					Q && n.iframe && (q.forceStyles(n.pd.documentElement, l), q.forceStyles(n.pd.body, l), q.forceStyles(n.iframe, e.extend(l, {
						width: "100%",
						height: "100%",
						top: 0,
						left: 0,
				...
				_exit: function(c) {
					n.active = !1;
					var e = !a._options.trueFullscreen || !S.support,
						d = a.$("container").removeClass("fullscreen");
					n.parent ? n.parent.prepend(d) : d.insertAfter(n.prev);
					if (e && (q.hide(a.getActiveImage()), q.revertStyles(a.get("container"), J().html, J().body), h.TOUCH || i.scrollTo(0, n.scrolled), (d = a._controls.frames[a._controls.active]) && d.image)) d.image.src = d.image.src;
			...
			t = this._idle = {
				...
				hide: function() {
					if (a._options.idleMode && a.getIndex() !== !1) {
						...
						e.each(t.trunk, function(e, d) {
							var f = d.data("idle");
							if (f) d.data("idle").complete = !1, q.animate(d, f.to, {
				...
				show: function(c) {
					var f = c.data("idle");
					if (!t.active || !f.busy && !f.complete) f.busy = !0, a.trigger(h.IDLE_EXIT), a.clearTimer("idle"), q.animate(c, f.from, {
			...
			p = this._lightbox = {
				...
				init: function() {
					if (!p.initialized) {
						...
						var c = {},
							...
							d = "",
							f = {
								overlay: "position:fixed;display:none;opacity:" + f.overlayOpacity + ";filter:alpha(opacity=" + f.overlayOpacity * 100 + ");top:0;left:0;width:100%;height:100%;background:" + f.overlayBackground + ";z-index:99990",
							...
							g = {},
							i = "",
							i = y > 7 ? y < 9 ? "background:#000;filter:alpha(opacity=0);" : "background:rgba(0,0,0,0);" : "z-index:99999";
						f.nextholder += i;
						f.prevholder += i;
						...
						e.each(["Prev", "Next"], function(a, d) {
							var f = e(c[d.toLowerCase()]).html(/v/.test(d) ? "&#8249;&#160;" : "&#160;&#8250;"),
								g = e(c[d.toLowerCase() + "holder"]);
				...
				rescale: function(c) {
					var f = r.min(z.width() - 40, p.width),
						d = r.min(z.height() - 60, p.height),
						d = r.min(f / p.width, d / p.height),
						f = r.round(p.width * d) + 40,
						d = r.round(p.height * d) + 60,
						f = {
							width: f,
							height: d,
							"margin-top": r.ceil(d / 2) * -1,
							"margin-left": r.ceil(f / 2) * -1
						};
					c ? e(p.elems.box).css(f) : e(p.elems.box).animate(f, {
				...
				show: function(c) {
					...
					if (!p.keymap) p.keymap = e.extend({}, a._keyboard.map), a.attachKeyboard({
						escape: p.hide,
						right: p.showNext,
						left: p.showPrev
					});
					...
					p.image.load(f.big || f.image || f.iframe, function(g) {
						if (g.isIframe) {
							var h = e(i).width(),
								l = e(i).height();
						...
						p.elems.counter.innerHTML = c + 1 + " / " + d;
						z.resize(p.rescale);
						p.rescale();
						if (f.image && f.iframe) {
							...
							e(g.image).css("cursor", "pointer").mouseup(function(a, d) {
								return function(c) {
									...
									c.preventDefault();
									d.isIframe = !0;
									d.load(a.iframe + (a.video ? "&autoplay=1" : ""), {
										width: "100%",
										height: y < 8 ? e(p.image.container).height() : "100%"
									})
								}
							}(f, g))
						}
					});
					e(p.elems.overlay).show().css("visibility", "visible");
					e(p.elems.box).show()
				}
			},
			s = this._timer = {
	...
	h.prototype = {
		...
		_init: function() {
			...
			this.bind(h.DATA, function() {
				...
				if (a._options.height < 2) a._userRatio = a._ratio = a._options.height;
				var e = {
					width: 0,
					height: 0
				},
				...
				q.wait({
					...
					error: function() {
						f() ? h.raise("Could not extract sufficient width/height of the gallery container. Traced measures: width:" + e.width + "px, height: " + e.height + "px.", !0) : h.raise("Could not extract a stage height from the CSS. Traced height: " + f() + "px.", !0)
					},
			...
			this._options.swipe || e.each(Array(2), function(c) {
				...
				e(f.container).css({
					...
					top: 0,
					left: 0
				}).prepend(a._layers[c] = e(q.create("galleria-layer")).css({
					position: "absolute",
					top: 0,
					left: 0,
					right: 0,
				...
				var g = new h.Picture;
				g.isIframe = !0;
				e(g.container).attr("class", "galleria-frame").css({
					position: "absolute",
					top: 0,
					left: 0,
					zIndex: 4,
			...
			if (c.swipe) this.$("images").css({
				position: "absolute",
				top: 0,
				left: 0,
				width: 0,
				height: "100%"
			}), this.finger = new h.Finger(this.get("stage"), {
				onchange: function(c) {
					a.pause().show(c)
				},
				oncomplete: function(c) {
					var c = r.max(0, r.min(parseInt(c, 10), a.getDataLength() - 1)),
						f = a.getData(c);
				...
				if (c) if (c.iframe) {
					a.isPlaying() && a.pause();
					var f = a._controls.frames[a._active],
						g = a._stageWidth,
						h = a._stageHeight;
					e(f.container).find("iframe").length || (e(f.container).css({
						width: g,
						height: h,
						opacity: 0
					}).show().animate({
						...
						f.load(c.iframe + (c.video ? "&autoplay=1" : ""), {
							width: g,
							height: h
						}, function(e) {
							...
							e.scale({
								...
								iframelimit: c.video ? a._options.maxVideoSize : u
							})
						})
					}, 100))
				} else if (c.link) a._options.popupLinks ? i.open(c.link, "_blank") : i.location.href = c.link
			...
			e.each(["info", "counter", "image-nav"], function(e, f) {
			...
			e.each(h.on.binds, function(c, f) {
				e.inArray(f.hash, a._binds) == -1 && a.bind(f.type, f.callback)
			});
			return this
		},
		addTimer: function() {
		...
		_getWH: function() {
			...
			e.each(["width", "height"], function(e, i) {
				g._options[i] && typeof g._options[i] === "number" ? f[i] = g._options[i] : (h = [q.parseValue(a.css(i)), q.parseValue(c.css(i)), a[i](), c[i]()], g["_" + i] || h.splice(h.length, q.parseValue(a.css("min-" + i)), q.parseValue(c.css("min-" + i))), f[i] = r.max.apply(r, h))
			});
		...
		_createThumbnails: function(a) {
			...
			var c, g, f, l = this,
				n = this._options,
				t = a ? this._data.length - a.length : 0,
				p = t,
				r = [],
				w = 0,
				...
				s = typeof n.thumbnails === "string" ? n.thumbnails.toLowerCase() : null,
				z = function(a) {
				...
				A = function(a, d) {
					...
					l.trigger({
						...
						thumbTarget: a.image,
						index: a.data.order,
						galleriaData: l.getData(a.data.order)
					});
					...
					a.scale({
						...
						complete: function(a) {
							...
							e.each(["Width", "Height"], function(d, h) {
								f = h.toLowerCase();
								if (n.thumbCrop !== !0 || n.thumbCrop === f) g = {}, g[f] = a[f], e(a.container).css(g), g = {}, g[c[d]] = 0, e(a.image).css(g);
			...
			for (; this._data[t]; t++) g = this._data[t], a = g.thumb || g.image, (n.thumbnails === !0 || s == "lazy") && (g.thumb || g.image) ? (c = new h.Picture(t), c.index = t, c.displayed = !1, c.lazy = !1, c.video = !1, this.$("thumbnails").append(c.container), f = e(c.container), f.css("visibility", "hidden"), c.data = {
				width: q.parseValue(z("width")),
				height: q.parseValue(z("height")),
				order: t,
				src: a
			}, n.thumbCrop !== !0 ? f.css({
				width: "auto",
			...
			}), s == "lazy" ? (f.addClass("lazy"), c.lazy = !0, c.load(x, {
				height: c.data.height,
				width: c.data.width
			})) : c.load(a, u), n.preload === "all" && c.preload(g.image)) : g.iframe && s !== null || s === "empty" || s === "numbers" ? (c = {
			...
			}, s === "numbers" && e(c.image).text(t + 1), g.iframe && e(c.image).addClass("iframe"), this.$("thumbnails").append(c.container), i.setTimeout(v(c.image, t, c.container), 50 + t * 20)) : c = {
				container: null,
				image: null
			}, e(c.container).add(n.keepSource && n.linkSourceImages ? g.original : null).data("index", t).on(n.thumbEventType, D).data("thumbload", u), d === a && e(c.container).addClass("active"), this._thumbnails.push(c);
			r = this._thumbnails.slice(p);
			return this
		},
		lazyLoad: function(a, c) {
			var g = a.constructor == Array ? a : [a],
				f = this,
				h = 0;
			e.each(g, function(a, i) {
				if (!(i > f._thumbnails.length - 1)) {
					var p = f._thumbnails[i],
						o = p.data,
						q = function() {
		...
		lazyLoadChunks: function(a, c) {
			for (var e = this.getDataLength(), f = 0, h = 0, n = [], t = [], p = this, c = c || 0; f < e; f++) if (t.push(f), ++h == a || f == e - 1) n.push(t), h = 0, t = [];
			var o = function(a) {
					var e = n.shift();
			...
			q.wait({
				...
				success: function() {
					...
					if (a._options.swipe) {
						...
						e.each(Array(a.getDataLength()), function(g) {
							var f = new h.Picture,
								i = a.getData(g);
							e(f.container).css({
								position: "absolute",
								top: 0,
								left: a._stageWidth * g
							}).prepend(a._layers[g] = e(q.create("galleria-layer")).css({
								position: "absolute",
								top: 0,
								left: 0,
								right: 0,
		...
		load: function(a, c, g) {
			...
			e.isArray(a) ? this.validate(a) ? this._data = a : h.raise("Load failed: JSON Array not valid.") : (c += ",.video,.iframe", e(a).find(c).each(function(a, c) {
				...
				if (i) h.big = i;
				e.each("big title description link layer image".split(" "), function(a, d) {
					c.data(d) && (h[d] = c.data(d).toString())
				});
				if (!h.big) h.big = h.image;
				f._data.push(e.extend({
					title: c.attr("title") || "",
					thumb: c.attr("src"),
					image: c.attr("src"),
					big: c.attr("src"),
		...
		_parseData: function(a) {
			...
			e.each(this._data, function(a, f) {
				...
				if ("video" in f) {
					var i = da(f.video);
					if (i) g.iframe = (new aa(i.provider, i.id)).embed() +
					function() {
						...
						if (e == "image" && !c._options.videoPoster) g.image = u;
						else {
		...
		addPan: function(a) {
			if (this._options.imageCrop !== !1) {
				var a = e(a || this.getActiveImage()),
					...
					g = a.width() / 2,
					f = a.height() / 2,
					h = parseInt(a.css("left"), 10),
					i = parseInt(a.css("top"), 10),
					o = h || 0,
					p = i || 0,
					s = 0,
					z = 0,
					...
					D = 0,
					A = function(c, d, e) {
						if (c > 0 && (D = r.round(r.max(c * -1, r.min(0, d))), v !== D)) if (v = D, y === 8) a.parent()["scroll" + e](D * -1);
						else c = {}, c[e.toLowerCase()] = D, a.css(c)
					},
					u = function(a) {
						if (!(q.timestamp() - d < 50)) x = !0, g = c.getMousePosition(a).x, f = c.getMousePosition(a).y
					};
				y === 8 && (a.parent().scrollTop(p * -1).scrollLeft(o * -1), a.css({
					top: 0,
					left: 0
				}));
				this.$("stage").off("mousemove", u).on("mousemove", u);
				this.addTimer("pan" + c._id, function() {
					x && (s = a.width() - c._stageWidth, z = a.height() - c._stageHeight, h = g / c._stageWidth * s * -1, i = f / c._stageHeight * z * -1, o += (h - o) / c._options.imagePanSmoothness, p += (i - p) / c._options.imagePanSmoothness, A(z, p, "Top"), A(s, o, "Left"))
				}, 50, !0);
				return this
			}
		},
		...
		append: function(a) {
			var c, e;
			for (c in a) if (a.hasOwnProperty(c)) if (a[c].constructor === Array) for (e = 0; a[c][e]; e++) this.appendChild(c, a[c][e]);
			...
			if (a = a || this._controls.getActive()) {
				var g, f = function(a) {
						e(a.container).children(":first").css({
							top: r.max(0, q.parseValue(a.image.style.top)),
							left: r.max(0, q.parseValue(a.image.style.left)),
							width: q.parseValue(a.image.width),
							height: q.parseValue(a.image.height)
						})
					},
					c = e.extend({
				...
				this._options.layerFollow && this._options.imageCrop !== !0 ? typeof c.complete == "function" ? (g = c.complete, c.complete = function() {
					g.call(a, a);
					f(a)
				}) : c.complete = f : e(a.container).children(":first").css({
					top: 0,
					left: 0
				});
				a.scale(c);
		...
		rescale: function(a, c, g) {
			...
			(function() {
				...
				f._controls.frames[f._controls.active] && f._controls.frames[f._controls.active].scale({
					width: f._stageWidth,
					height: f._stageHeight,
					iframelimit: f._options.maxVideoSize
				});
				f.trigger(h.RESCALE);
		...
		_preload: function() {
			if (this._options.preload) {
				var a, c, e = this.getNext(),
					f;
				try {
					for (c = this._options.preload; c > 0; c--) a = new h.Picture, f = this.getData(e), a.preload(this.isFullscreen() && f.big ? f.big : f.image), e = this.getNext(e)
				} catch (i) {}
			}
		},
		show: function(a, c, g) {
			var f = this._options.swipe;
			if (f || !(this._queue.length > 3 || a === !1 || !this._options.queue && this._queue.stalled)) if (a = r.max(0, r.min(parseInt(a, 10), this.getDataLength() - 1)), c = typeof c !== "undefined" ? !! c : a < this.getIndex(), !g && h.History) h.History.set(a.toString());
			else {
				...
				if (f) {
					var l = this.getData(a),
						n = this;
					if (!l) return;
					var o = this.isFullscreen() && l.big ? l.big : l.image || l.iframe,
						p = this._controls.slides[a],
						q = {
							cached: p.isCached(o),
							index: a,
							rewind: c,
							imageTarget: p.image,
							thumbTarget: this._thumbnails[a].image,
							galleriaData: l
						};
					this.trigger(e.extend(q, {
						type: h.LOADSTART
					}));
					n.$("container").removeClass("videoplay");
					var s = function() {
							n._layers[a].innerHTML = n.getData().layer || "";
							n.trigger(e.extend(q, {
								type: h.LOADFINISH
							}));
							n._playCheck()
						};
					n._preload();
					i.setTimeout(function() {
						if (!p.ready || e(p.image).attr("src") != o) {
							if (l.iframe && !l.image) p.isIframe = !0;
							p.load(o, function(a) {
								q.imageTarget = a.image;
								n._scaleImage(a, s).trigger(e.extend(q, {
									type: h.IMAGE
								}));
								s()
							})
						} else n.trigger(e.extend(q, {
							type: h.IMAGE
						})), s()
					}, 100)
		...
		_show: function() {
			var a = this,
				c = this._queue[0],
				g = this.getData(c.index);
			if (g) {
				var f = this.isFullscreen() && g.big ? g.big : g.image || g.iframe,
					l = this._controls.getActive(),
					n = this._controls.getNext(),
					o = n.isCached(f),
					p = this._thumbnails[c.index],
					s = function() {
				...
				var r = function(c, d, f, g, l) {
						return function() {
							...
							if (c.iframe && c.image || c.link || a._options.lightbox || a._options.clicknext) e(d.image).css({
								cursor: "pointer"
							}).on("mouseup", function(d) {
								if (!(typeof d.which == "number" && d.which > 1)) if (c.iframe) {
									a.isPlaying() && a.pause();
									var f = a._controls.frames[a._controls.active],
										g = a._stageWidth,
										l = a._stageHeight;
									e(f.container).css({
										width: g,
				...
				n.load(f, function(f) {
					...
					a._scaleImage(f, {
						complete: function(f) {
							"image" in l && q.toggleQuality(l.image, !1);
		...
		_playCheck: function() {
			...
			if (this._playing) {
				...
				var i = function() {
						c = q.timestamp() - e;
						c >= a._playtime && a._playing ? (a.clearTimer(f), a.next()) : a._playing && (a.trigger({
							type: h.PROGRESS,
							percent: r.ceil(c / a._playtime * 100),
							seconds: r.floor(c / 1E3),
							milliseconds: c
						}), a.addTimer(f, i, 20))
					};
				a.addTimer(f, i, 20)
	...
	e.extend(h, {
		...
		IE6: y === 6,
		IE: y,
		WEBKIT: /webkit/.test(v),
		CHROME: /chrome/.test(v),
		SAFARI: /safari/.test(v) && !/chrome/.test(v),
		QUIRK: y && o.compatMode && o.compatMode === "BackCompat",
		MAC: /mac/.test(navigator.platform.toLowerCase()),
		OPERA: !! i.opera,
		IPHONE: /iphone/.test(v),
		IPAD: /ipad/.test(v),
		ANDROID: /android/.test(v),
		...
		if (!e("script").filter(function() {
			...
			e(i).load(function() {
				c || (g = i.setTimeout(function() {
					c || h.raise("Galleria had problems loading theme at " + a + ". Please check theme path or load manually.", !0)
				}, 2E4))
			});
	...
	h.get = function(a) {
		if (N[a]) return N[a];
		else if (typeof a !== "number") return N;
		else h.raise("Gallery index " + a + " not found")
	};
	...
	h.on = function(a, c) {
		if (a) {
			var c = c || I,
				g = a + c.toString().replace(/\s/g, "") + q.timestamp();
		...
		var g = c ? "Fatal error" : "Error",
			...
			h = function(a) {
				...
				e.each(N, function() {
					var a = this.$("errors"),
						c = this.$("target");
					a.length || (c.css("position", "relative"), a = this.addElement("errors").appendChild("target", "errors").$("errors").css(f));
					a.append(h)
				});
				N.length || e("<div>").css(e.extend(f, {
					position: "fixed"
				})).append(h).appendTo(J().body)
			};
		if (s) {
	...
	h.Picture.prototype = {
		...
		load: function(a, c, g) {
			...
			h.IE8 && e(this.image).css("filter", "inherit");
			...
			s.load(r).on("error", function() {
				n ? C ? e(this).attr("src", C) : h.raise("Image not found: " + a) : (n = !0, i.setTimeout(function(a, c) {
					return function() {
		...
		scale: function(a) {
			...
			if (this.isIframe) {
				...
				if (a.iframelimit) {
					...
					o < 1 ? (i = g * o, n = f * o, e(this.image).css({
						top: f / 2 - n / 2,
						left: g / 2 - i / 2,
						position: "absolute"
					})) : e(this.image).css({
			...
			q.wait({
				...
				success: function() {
					var f = (s - a.margin * 2) / c.original.width,
						g = (z - a.margin * 2) / c.original.height,
						h = r.min(f, g),
						i = r.max(f, g),
						l = {
							"true": i,
							...
							height: g,
							"false": h,
							landscape: c.original.width > c.original.height ? i : h,
							portrait: c.original.width < c.original.height ? i : h
						}[a.crop.toString()],
						f = "";
					...
					if (a.canvas && V) V.elem.width = c.width, V.elem.height = c.height, f = c.image.src + ":" + c.width + "x" + c.height, c.image.src = V.cache[f] ||
					function(a) {
					...
					e(c.image).css({
						position: "absolute",
						top: f(n.top, "height", z),
						left: f(n.left, "width", s)
					});
					c.show();
					c.ready = !0;
					a.complete.call(c, c)
	...
	e.extend(e.easing, {
		galleria: function(a, c, e, f, h) {
			if ((c /= h / 2) < 1) return f / 2 * c * c * c + e;
			return f / 2 * ((c -= 2) * c * c + 2) + e
		},
		...
		g.prototype = {
			...
			ontouchmove: function(a) {
				...
				if (!(c && c.length > 1 || a.scale && a.scale !== 1)) {
					...
					if (!this.isScrolling) a.preventDefault(), this.deltaX /= !this.index && this.deltaX > 0 || this.index == this.length - 1 && this.deltaX < 0 ? r.abs(this.deltaX) / this.width + 1.8 : 1, this.to = this.deltaX - this.index * this.width;
					a.stopPropagation()
				}
			},
			ontouchend: function() {
				this.touching = !1;
				var a = +new Date - this.start.time < 250 && r.abs(this.deltaX) > 40 || r.abs(this.deltaX) > this.width / 2,
			...
			loop: function() {
				...
				if (this.touching || r.abs(a) <= 1) {
					...
					c = this.config.duration * this.anim.factor;
	...
	e.fn.galleria = function(a) {
		...
		if (!e(this).length) return e(function() {
			e(c).length ? e(c).galleria(a) : h.utils.wait({
				until: function() {
					return e(c).length
				},
				success: function() {
...
(function(e, i, h, u) {
	(function(e) {
		...
		var o = /\+/g,
			v = e.cookie = function(o, C, r) {
				if (C !== u) {
					r = e.extend({}, v.defaults, r);
					if (typeof r.expires === "number") {
						var B = r.expires,
							M = r.expires = new Date;
						M.setDate(M.getDate() + B)
					}
		...
		e.removeCookie = function(h, i) {
			if (e.cookie(h) !== u) return e.cookie(h, "", e.extend({}, i, {
				expires: -1
			})), !0;
			return !1
		}
	...
	var o = [],
		B = e.G = {
			...
			views: {
				...
				settings: function() {
					var h = i("form.settings"),
						o = h.find(".verify").hide(),
						s = h.find(".email"),
						u = s.val(),
						v;
					s.focus(function() {
				...
				"theme-purchase": function() {
					...
					v.blur(function() {
						v.val().length ? B || (o = !0, v.removeClass("error"), D.html("")) : (o = !1, D.html("This field is required."), v.attr("class", "error"))
					});
					...
					i("input[type=image]").each(function() {
						s = s.toFixed(2);
						var r = '<div class="total">Total: <span>' + (i(this).closest(".buy-form").hasClass("hasvat") ? "$" + s * 0.8 + " + $" + (s * 0.2).toFixed(2) + " VAT = <strong>$" + s : "<strong>$" + s) + "</strong></span></div>",
							u = i("<a>", {
								href: "#",
								html: "Buy Now",
								click: function(s) {
									...
									i("div.customer").length ? i.post(h.attr("action"), h.serialize(), function(e) {
										...
										i.post(h.attr("action"), h.serialize(), function(h) {
											if (h.errors) if (r.bind("click", u).parent().removeClass("disabled"), h.errors.email) B = !0, D.html(h.errors.email[0]), v.attr("class", "error").focus();
											else {
												var o = i("<div>").addClass("messages"),
													s = i("<ul>").appendTo(o);
...
(function(e) {
	G.make(0, {
		...
		init: function(i) {
			...
			var h = this,
				u = window.document,
				o = i._locale,
				B = "getContext" in u.createElement("canvas");
			(function() {
				if (B) {
					var i = u.createElement("canvas"),
						o = i.getContext("2d");
					i.width = 24;
					i.height = 24;
					e(i).css({
			...
			this.addIdleState(this.get("info"), z, Galleria.IE < 9 ? {} : {
				opacity: 1
				...
				opacity: 1
			}, !0).addIdleState(this.get("counter"), A, Galleria.IE < 9 ? {} : {
				opacity: 0.9
			}, !0);
			...
			i._toggleCaption && (this.$("info").addClass("toggler"), this.addElement("captionopen").appendChild("stage", "captionopen"), this.addElement("captionclose").appendChild("info", "captionclose"), this.$("captionopen").on("click:fast", function() {
				...
				e(this).hide()
			}).html(o.show_captions), this.bind("loadstart", function() {
			...
			i._showTooltip && this.bindTooltip({
				fullscreen: function() {
				...
				"image-nav-right": o.next,
				"image-nav-left": o.prev,
				counter: function() {
		...
		init: function(i) {
			...
			var h = this,
				...
				J = function() {
					...
					Galleria.utils.animate(u, {
						top: K ? h.getStageHeight() + 30 : 0
					}, {
						easing: "galleria",
						duration: 400,
						complete: function() {
			...
			this.bind("fullscreen_enter", function() {
				...
				Galleria.TOUCH || this.addIdleState(s, {
					bottom: -31
				})
			});
			this.bind("fullscreen_exit", function() {
				...
				Galleria.TOUCH || this.removeIdleState(s, {
					bottom: -31
				})
			});
			this.bind("rescale", y);
	...
	G.make(2, {
		...
		init: function(i) {
			...
			var h = this.$("thumbnails-tab"),
				u = this.$("loader"),
				o = this.$("thumbnails-container"),
				B = this.$("thumbnails-list"),
				z = this.$("info-text"),
				A = this.$("info"),
				s = !i._hideDock,
				C = 0;
			...
			Galleria.TOUCH || (this.addIdleState(o, {
				opacity: 0
			...
			if (i._hideDock) h.on("click:fast", this.proxy(function() {
				...
				s ? o.animate({
					top: C
				}, 400, i.easing) : o.animate({
					top: C - B.outerHeight() + 2
				}, 400, i.easing);
				s = !s
			}));
			else this.bind("thumbnail", function() {
	...
	G.make(3, {
		...
		init: function(i) {
			...
			var h = this.$("info-link,info-close,info-text"),
				u = Galleria.TOUCH;
			this.$("loader,counter").show().css("opacity", 0.4);
			u || (this.addIdleState(this.get("image-nav-left"), {
				left: -50
			}), this.addIdleState(this.get("image-nav-right"), {
				right: -50
			...
			this.bind("thumbnail", function(h) {
				u ? e(h.thumbTarget).css("opacity", this.getIndex() ? 1 : 0.6).bind("click:fast", function() {
					...
					e(this).not(".active").children().stop().fadeTo(400, 0.6)
				}), h.index === this.getIndex() && e(h.thumbTarget).css("opacity", 1))
			});
			this.bind("loadstart", function(h) {
	...
	G.make(4, {
		...
		init: function(i) {
			...
			var h = this,
				...
				r = 0,
				I = 0,
				M = window.location.hash.substr(2),
				y = function(i) {
					h.$("info").css({
						left: h.finger ? 20 : Math.max(20, e(window).width() / 2 - i / 2 + 10)
					})
				},
				J = function(h, i) {
					var i = e.extend({
					...
					if (h.data("colCount") !== u && (h.data("colCount", u), s.length)) {
						...
						!(s < 0) && typeof s === "number" && (i.speed ? h.animate({
							height: Math.max.apply(window, z)
						}, i.speed, i.onheight) : (h.height(Math.max.apply(window, z)), i.onheight.call(h)))
					}
				};
			...
			this._fullscreen.beforeExit = function(e) {
				A.hide();
				Galleria.IE8 && Galleria.utils.animate(h.getActiveImage(), {
					opacity: 0
				}, {
					duration: 200
				});
				Galleria.utils.animate(u[0], {
					opacity: 0
				}, {
					duration: 200,
					complete: function() {
			...
			this.bind("thumbnail", function(s) {
				...
				v === this.getDataLength() && (this.$("preloader").fadeOut(100), o.data("colCount", null), J(o, {
					width: r,
					speed: i._animate ? 400 : 0,
					onbrick: function() {
						...
						window.setTimeout(function(r) {
							return function() {
								...
								r.parent().off("click:fast click").on("click:fast", function() {
									...
									e.isFunction(i._onClick) ? i._onClick.call(h, h.getData(r)) : (u.css({
										visibility: "visible",
										opacity: 0
									}), h.$("target").height(h.$("target").height()), h.finger && z.css("opacity", 0), Galleria.utils.animate(o[0], {
										opacity: 0
									}, {
										duration: 100,
										complete: function() {
			...
			Galleria.History && M && (u.css("visibility", "visible"), o.css("opacity", 0), this.$("preloader").hide(), this.enterFullscreen(function() {
				this.show(parseInt(M, 10))
			}));
			var Q = null,
				R = function() {
					Q = null;
					if (h.isFullscreen()) h.getActiveImage() && y(h.getActiveImage().width);
					else {
						var e = B.width();
						e !== D && (D = e, J(o, {
							width: r,
	...
	G.make(5, {
		...
		init: function(i) {
			...
			var s = function(i) {
					return e("<div>").html("&#8226;").on("click:fast", function(e) {
						return function(i) {
							i.preventDefault();
							h.show(e)
						}
			...
			this.bindTooltip({
				...
				desc: i._locale.click_to_close,
				more: i._locale.show_info,
				thumbs: i._locale.show_thumbnails
			});
			this.bind("loadstart", function(e) {
				e.cached || this.$("loader").show().fadeTo(200, 0.4)
			});
			this.bind("loadfinish", function(e) {
				var o = h.getData().title,
					s = h.getData().description;
				z.fadeOut(200);
